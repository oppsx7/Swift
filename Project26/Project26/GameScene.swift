...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func loadLevel() {
        ...
        for (row, line) in lines.reversed().enumerated() {
            for (column, letter) in line.enumerated() {
                ...
                if letter == "x" {
                    createNode(nodeName: "block", enumType: .wall, position: position)
                    
                } else if letter == "v" {
                    createNode(nodeName: "vortex", enumType: .vortex, position: position)
                    
                } else if letter == "s" {
                    createNode(nodeName: "star", enumType: .star, position: position)
                    
                } else if letter == "f" {
                    createNode(nodeName: "finish", enumType: .finish, position: position)
                } else if letter == " " {
    ...
    func createNode(nodeName: String, enumType: CollisionTypes, position: CGPoint) {
        ...
        } else if nodeName == "vortex" || nodeName == "finish" || nodeName == "star" {
            node.physicsBody = SKPhysicsBody(circleOfRadius: node.size.width / 2)
            node.physicsBody?.contactTestBitMask = CollisionTypes.player.rawValue
            node.physicsBody?.collisionBitMask = 0
        }
        node.physicsBody?.categoryBitMask = enumType.rawValue
